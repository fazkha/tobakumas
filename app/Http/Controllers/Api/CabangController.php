...
class CabangController extends Controller
{
    ...
    public function loadPengeluaranBulanan(Request $request)
    {
        ...
        $rekap = DB::select("CALL sp_pc_pengeluaran_bulanan(?,?,?)", [$data['id'], $data['bulan'], $data['tahun']]);
    ...
    public function gerobakAktif(Request $request)
    {
        ...
        try {
            $rute = RuteGerobak::join('users', 'rute_gerobaks.user_id', '=', 'users.id')
                ...
                ->selectRaw('rute_gerobaks.latitude as latitude, rute_gerobaks.longitude as longitude, branches.nama as cabang, users.name as mitra, DATE(FROM_UNIXTIME(rute_gerobaks.timesaved)) as tanggal, TIME_FORMAT(TIME(FROM_UNIXTIME(rute_gerobaks.timesaved)), "%H:%i") as jam')
