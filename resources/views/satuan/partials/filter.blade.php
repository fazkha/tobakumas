<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
        <div
            ...
            <div class="flex flex-col lg:flex-row justify-start">
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option {{ session('satuan_pp') == config('custom.list_per_page_opt_1') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_1') }}">
                                {{ config('custom.list_per_page_opt_1') }}</option>
                            <option {{ session('satuan_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option {{ session('satuan_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('satuan_isactive') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('satuan_isactive') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('satuan_isactive') == '0' ? 'selected' : '' }} value="0">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.unitname')</span>
                        <input id="search-nama_lengkap" placeholder="{{ __('messages.search') }}"
                            value="{{ session('satuan_nama_lengkap') == '_' ? '' : session('satuan_nama_lengkap') }}"
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.abbreviation')</span>
                        <input id="search-singkatan" placeholder="{{ __('messages.search') }}"
                            value="{{ session('satuan_singkatan') == '_' ? '' : session('satuan_singkatan') }}"
                            class="text-sm pl-28 pr-6 pt-1.5 pb-2 appearance-none rounded-md border block w-full bg-primary-20 border-primary-100 placeholder-gray-400 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800" />
                    </div>
                </div>
            </div>
        ...
        <div>
            @can('satuan-create')
                <x-anchor-primary href="{{ route('units.create') }}">
