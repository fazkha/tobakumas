...
<x-app-layout>
    ...
    @push('scripts')
        ...
        <script>
            $("#pp-dropdown, #isactive-dropdown, #educlvl-dropdown, #search-title, #search-description, #search-price").on(
                "change keyup paste",
                function() {
                    var xpp = $('#pp-dropdown option:selected').val();
                    var xisactive = $('#isactive-dropdown option:selected').val();
                    var xcoasgroups = $('#coasgroups-dropdown option:selected').val();
                    ...
                    var newURL = '{{ url('/admin/coa') }}';
                    var newState = {
                        page: 'index-coa'
                    };
                    var newTitle = '{{ __('messages.chartofaccount') }}';

                    window.history.pushState(newState, newTitle, newURL);

                    $.ajax({
                        url: '{{ url('/admin/coa/fetchdb') }}' + "/" + xpp + "/" + xisactive + "/" + xcoasgroups +
                            "/" + xkode + "/" + xnama,
                        type: "GET",
                        dataType: 'json',
