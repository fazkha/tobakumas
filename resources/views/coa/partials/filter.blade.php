<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
            ...
            <div class="flex flex-col lg:flex-row justify-start">
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option {{ session('coa_pp') == config('custom.list_per_page_opt_1') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_1') }}">
                                {{ config('custom.list_per_page_opt_1') }}</option>
                            <option {{ session('coa_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option {{ session('coa_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('coa_isactive') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('coa_isactive') == '1' ? 'selected' : '' }} value="1">Yes</option>
                            <option {{ session('coa_isactive') == '0' ? 'selected' : '' }} value="0">No</option>
                        </select>
                        <div
                            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700 dark:text-white">
                        </div>
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="coasgroups-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('coa_coasgroups_id') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            @foreach ($coasgroups as $id => $name)
                                <option {{ session('coa_coasgroups_id') == $id ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-code" placeholder="@lang('messages.search')"
                            value="{{ session('coa_code') == '_' ? '' : session('coa_code') }}"
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-name" placeholder="@lang('messages.search')"
                            value="{{ session('coa_name') == '_' ? '' : session('coa_name') }}"
                            class="text-sm pl-28 pr-6 pt-1.5 pb-2 appearance-none rounded-md border block w-full bg-primary-20 border-primary-100 placeholder-gray-400 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800" />
                    </div>
        ...
        <div>
            @can('coa-create')
                <x-anchor-primary href="{{ route('coa.create') }}">
