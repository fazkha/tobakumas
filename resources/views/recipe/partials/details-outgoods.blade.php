@if ($outgoods->count() > 0)
    @foreach ($outgoods as $outgood)
        <tr>
            <td class="align-top">
                <x-text-span>{{ $outgood->barang->nama }}</x-text-span>
            </td>
            <td class="align-top">
            ...
            @if ($viewMode == false)
                <td class="align-top">
                    <x-anchor-danger id="a-delete-outgoods-{{ $outgood->id }}"
                        onclick="deleteOutgoods({{ $outgood->id }})" class="!px-1" title="{{ __('messages.delete') }}">
