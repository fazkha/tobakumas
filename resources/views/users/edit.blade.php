@php
    ...
    if ($profile) {
        $profile_branch = $profile->branch_id;
    } else {
        $profile_branch = null;
    }
@endphp
                    ...
                    <div class="p-6 space-y-2 md:space-y-2 sm:p-8">
                        <form action="{{ route('users.update', Crypt::Encrypt($datas->id)) }}"
                                ...
                                <x-text-span>
                                    <div class="px-2 pb-2">
                                        <div class="inline-flex items-center">
                                            <input type="checkbox" id="approved" name="approved" tabindex="3"
                                    ...
                                    <div class="px-2 pb-2">
                                        ...
                                        <select name="branch_id" id="branch_id" tabindex="4" required
                                            ...
                                            @foreach ($branches as $id => $name)
                                                <option value="{{ $id }}"
                                                    {{ old('branch_id', $profile_branch) == $id ? 'selected' : '' }}>
                                                    {{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="flex flex-col md:flex-row gap-2 justify-between">
                                ...
                                <x-anchor-secondary href="{{ route('users.index') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-5">
