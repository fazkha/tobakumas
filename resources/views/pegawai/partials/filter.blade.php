<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
        <div
            ...
            <div class="flex flex-col lg:flex-row justify-start">
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="pp-dropdown"
                            ...
                            <option
                                {{ session('pegawai_pp') == config('custom.list_per_page_opt_2') ? 'selected' : '' }}
                                value="{{ config('custom.list_per_page_opt_2') }}">
                                {{ config('custom.list_per_page_opt_2') }}</option>
                            <option
                                {{ session('pegawai_pp') == config('custom.list_per_page_opt_3') ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            ...
                            <option {{ session('pegawai_isactive') == '0' ? 'selected' : '' }} value="0">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.gender')</span>
                        <select id="kelamin-dropdown"
                            ...
                            <option {{ session('pegawai_kelamin') == '1' ? 'selected' : '' }} value="L">
                                @lang('messages.genderman')</option>
                            <option {{ session('pegawai_kelamin') == '0' ? 'selected' : '' }} value="P">
                                @lang('messages.genderwoman')</option>
                        </select>
                        <div
                            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700 dark:text-white">
                        </div>
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="cabang_id-dropdown"
                            ...
                            @foreach ($cabangs as $id => $name)
                                <option {{ session('barang_cabang_id') == $id ? 'selected' : '' }}
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.jobposition')</span>
                        <select id="jabatan_id-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('barang_jabatan_id') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            @foreach ($jabatans as $id => $name)
                                <option {{ session('barang_jabatan_id') == $id ? 'selected' : '' }}
                                    value="{{ $id }}">{{ $name }}</option>
                            @endforeach
                        </select>
                        <div
        ...
        <div>
            @can('pegawai-create')
                <x-anchor-primary href="{{ route('employee.create') }}">
