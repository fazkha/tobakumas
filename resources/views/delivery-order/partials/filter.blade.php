<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
        <div
            ...
            <div class="flex flex-col lg:flex-row justify-start">
                <div class="flex-row justify-start">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.packagedfinish')</span>
                        <select id="produksi-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('delivery-order_selesai') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('delivery-order_selesai') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('delivery-order_selesai') == '0' ? 'selected' : '' }} value="0">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-tanggal" type="date" placeholder="@lang('messages.search')"
                            value="{{ session('delivery-order_tanggal') == '_' ? '' : session('delivery-order_tanggal') }}"
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <input id="search-alamat" placeholder="@lang('messages.search')"
                            value="{{ session('delivery-order_alamat') == '_' ? '' : session('delivery-order_alamat') }}"
                            class="text-sm pl-28 pr-6 pt-1.5 pb-2 appearance-none rounded-md border block w-full bg-primary-20 border-primary-100 placeholder-gray-400 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800" />
                    </div>
                </div>
            </div>
        ...
        <div class="hidden">
            @can('delivery-create')
                <x-anchor-primary href="{{ route('delivery-order.create') }}">
