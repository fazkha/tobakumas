...
<x-app-layout>
    <div class="flex items-center justify-between px-4 py-4 border-b border-primary-100 lg:py-6 dark:border-primary-800">
        <a href="{{ route('purchase-plan.index') }}">
    ...
    <div class="mx-auto px-4 py-2">
        <div class="flex flex-col items-center">
            ...
            <div class="w-full">
                @include('purchase-plan.partials.filter')
            </div>

            <div id="table-container" class="w-full">
                @include('purchase-plan.partials.table')
            </div>
    ...
    @push('scripts')
        ...
        <script>
            $("#pp-dropdown, #isactive-dropdown, #bulan-dropdown, #supplier-dropdown, #search-tahun")
                .on(
                    "change keyup paste",
                    function() {
                        ...
                        var newURL = '{{ url('/purchase/plan') }}';
                        var newState = {
                            page: 'index-purchase-plan'
                        };
                        var newTitle = '{{ __('messages.purchaseplan') }}';

                        window.history.pushState(newState, newTitle, newURL);

                        $.ajax({
                            url: '{{ url('/purchase/plan/fetchdb') }}' + "/" + xpp + "/" + xisactive + "/" +
                                xsupplier + "/" + xbulan + "/" + xtahun,
                            type: "GET",
                            dataType: 'json',
