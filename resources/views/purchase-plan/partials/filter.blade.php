<div class="my-2">
    <div class="flex flex-col-reverse md:flex-row gap-4 justify-between">
                ...
                <div class="flex-row justify-start">
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="isactive-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('purchase-plan_isactive') == 'all' ? 'selected' : '' }} value="all">
                                @lang('messages.all')</option>
                            <option {{ session('purchase-plan_isactive') == '1' ? 'selected' : '' }} value="1">
                                @lang('messages.yes')</option>
                            <option {{ session('purchase-plan_isactive') == '0' ? 'selected' : '' }} value="0">
                ...
                <div class="flex-row justify-start">
                    <div class="relative shadow-md mr-2 mb-2">
                        ...
                        <select id="supplier-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('purchase-plan_supplier_id') == 'all' ? 'selected' : '' }}
                                value="all">
                                @lang('messages.all')
                            </option>
                            @foreach ($suppliers as $id => $name)
                                <option {{ session('purchase-plan_supplier_id') == $id ? 'selected' : '' }}
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.month')</span>
                        <select id="bulan-dropdown"
                            class="text-sm px-2 leading-tight pl-28 pr-9 py-2 appearance-none w-full h-full rounded-md border block bg-primary-20 border-primary-100 text-gray-700 dark:text-white dark:bg-primary-700 dark:border-primary-800">
                            <option {{ session('purchase-plan_periode_bulan') == 'all' ? 'selected' : '' }}
                                value="all">
                                @lang('messages.all')
                            </option>
                            @foreach ($bulans as $id => $name)
                                <option {{ session('purchase-plan_periode_bulan') == $id ? 'selected' : '' }}
                                    value="{{ $id }}">{{ $name }}</option>
                            @endforeach
                        </select>
                        <div
                    ...
                    <div class="relative shadow-md mr-2 mb-2">
                        <span
                            class="w-24 text-xs h-full absolute inset-y-0 left-0 flex items-center px-2 border-r border-primary-100 dark:border-primary-800">@lang('messages.year')</span>
                        <input id="search-tahun" type="number" min="0" placeholder="@lang('messages.search')"
                            value="{{ session('purchase-plan_periode_tahun') == '_' ? '' : session('purchase-plan_periode_tahun') }}"
        ...
        <div>
            @can('po-create')
                <x-anchor-primary href="{{ route('purchase-plan.create') }}">
