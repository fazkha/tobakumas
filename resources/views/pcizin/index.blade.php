...
<x-app-layout>
    ...
    <div class="mx-auto px-4 py-2">
        <div class="flex flex-col items-center">
            ...
            <div class="w-full">
                @include('pcizin.partials.filter')
            </div>

            <div id="table-container" class="w-full">
                @include('pcizin.partials.table')
            </div>
        ...
        <script type="text/javascript">
            $("#pp-dropdown, #show-dropdown, #branch-dropdown, #mitra-dropdown, #search-tanggal").on(
                ...
                function() {
                    ...
                    var newURL = '{{ url('/human-resource/pcizin') }}';
                    var newState = {
                        page: 'index-pcizin'
                    };
                    var newTitle = '{{ __('messages.pcizin') }}';

                    window.history.pushState(newState, newTitle, newURL);

                    $.ajax({
                        url: '{{ url('/human-resource/pcizin/fetchdb') }}' + "/" + xpp + "/" + xshow + "/" +
                            xbr + "/" + xmt + "/" + xtanggal,
                        type: "GET",
                        dataType: 'json',
